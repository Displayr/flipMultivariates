...
{
    names <- colnames(data)
    all.vars.formula <- paste0(names[1], "~", paste0(names[2:ncol(data)], collapse = "+"))
...
#' @description Generate a covariance or correlation matrix from weighted or unweighted data
#'              using either the set of complete observations, or using pairwise-complete
#'              observations.
#'
#' @param data A data frame containing the input data.
#' @param weights A numeric vector containing the value of the weight for each row of \code{data}.
#'                If weights is NULL then this function is just a wrapper for the base functions
#'                \code{cov} and \code{cor}.
