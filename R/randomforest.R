...
print.RandomForest <- function(x, ...)
{
    ...
    {
        print.original.rf(x$original)
        invisible(x)
    }
}

# Copy of print.rf without the function call being printed
print.original.rf <- function(x, ...)
{
    #cat("\nCall:\n", deparse(x$call), "\n")
    ...
    if (x$type == "classification") {
        if (!is.null(x$confusion)) {
            cat("        OOB estimate of  error rate: ", round(x$err.rate[x$ntree,
                                                                          "OOB"] * 100, digits = 2), "%\n", sep = "")
    ...
    if (x$type == "regression") {
        if (!is.null(x$mse)) {
            cat("          Mean of squared residuals: ", x$mse[length(x$mse)],
                "\n", sep = "")
            cat("                    % Var explained: ", round(100 *
                                                                   x$rsq[length(x$rsq)], digits = 2), "\n", sep = "")
