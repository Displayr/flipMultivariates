...
#'
#' @param list.of.variables A variable in a \code{DataSet} or \code{data.frame}.
#' @param coerce.to.numeric Makes factors and ordered factors numeric.
#' @export
ListToDataFrame <- function(list.of.variables, coerce.to.numeric = FALSE)
{
    ...
    for (variable in list.of.variables)
    {
        ...
        if (is.character(variable))
            stop(paste("Variable", counter, "is a Text variable. It needs to be converted to numeric data if to be used in cluster analysis."))
        if (is.data.frame(variable))
        {
            transformed.variable <- ListToDataFrame(variable, coerce.to.numeric)
            colnames(transformed.variable) <- paste(variable.name, colnames(transformed.variable))
        }
        else
        {
